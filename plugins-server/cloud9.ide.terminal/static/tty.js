...
inherits(TtyManager, EventEmitter)
...
//            tty.socket = eio.Socket(server, { resource: resource });
            tty.socket = ide.connection.socket;
...
//            tty.socket = new eio.Socket();
        ...
        tty.socket.on('message', function(data) {
            ...
            if(data.cmd == 'data') {
                ...
                tty.handleKillACK(data)
            } else if (data.cmd == 'sync') {
                tty.handleSync(data)
            } else if (data.cmd == "createACK") {

            } else if (data.cmd == "processACK") {
                var tab_id = data.id
    ...
    tty.handleSync = function(data) {
        ...
        Object.keys(data.terms).forEach(function(key) {
            ...
            var tdata = data.terms[key]
                , win = new Window({transport:transport, resume: true })
                , tab = win.tabs[0];
    ...
    tty.handleKillACK = function(data) {
        if(!tty.terms[data.id]) return;
    ...
    function Window(options) {
        ...
        this.cols = options.rows || Terminal.geometry[0];
        this.rows = options.cols || Terminal.geometry[1];
...
//            }
    ...
    Window.prototype.drag = function(ev) {
        ...
        function up() {
            ...
            self.sendData(JSON.stringify({cmd: 'move', id: id, left: el.style.left, top: el.style.top}));
    ...
    Window.prototype.sendData = function(data) {
...
//        x = this.parentElement.clientWidth / term.element.offsetWidth;
//        y = this.parentElement.clientHeight / term.element.offsetHeight;
    ...
    Window.prototype.onMessage = function(cb) {
    ...
    function Tab(win) {
        ...
        if (!this.window.resume) {
            this.sendData(JSON.stringify({cmd: 'create', cols: cols, rows: rows}));
            this.onMessage(function(data) {
                data = JSON.parse(data);
                if (data.cmd == 'createACK') {
                   self.handleCreateACK(data)
                }
            })
        }
...
// in the constructor, but this is faster.
    Tab.prototype.handler = function(data) {
        this.sendData(JSON.stringify({cmd: 'data', id: this.id, payload: data}));
    };

    Tab.prototype.sendData = function(data) {
        this.window.sendData(data)
    };

    Tab.prototype.handleCreateACK = function(data) {
        var self = this
        if (self.id == '') {
...
// in the constructor, but this is faster.
    ...
    Tab.prototype.resize = function(cols, rows) {
        this.sendData(JSON.stringify({cmd: 'resize', id: this.id, cols: cols, rows: rows}));
    ...
    Tab.prototype.destroy = function() {
        if (this.destroyed) return;
        this.sendData(JSON.stringify({cmd: 'kill', id: this.id}));
        this._destroy();
    ...
    Tab.prototype.hookKeys = function() {
        ...
        this.on('request paste', function(key) {
            self.sendData(JSON.stringify({cmd: 'request paste'}));
    ...
    Tab.prototype.onMessage = function(cb) {
        this.window.onMessage(cb)
    }
    ...
    Tab.prototype.pollProcessName = function(func) {
        var self = this;
        self.sendData(JSON.stringify({cmd: 'process', id: this.id}));
